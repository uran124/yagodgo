...
<div class="bg-white dark:bg-slate-900 p-6 rounded shadow max-w-4xl mx-auto">
  ...
  <div id="tab-desc" class="tab-content space-y-4 hidden">
    ...
    <fieldset class="md:col-span-2 rounded border border-[#C86052] p-4"><legend class="px-2 font-semibold">Внешние каталоги</legend><label class="flex items-center gap-2"><input type="checkbox" name="external_catalog_enabled" value="1" <?= !empty($product['external_catalog_enabled']) ? 'checked' : '' ?>> Выгружать во внешние каталоги</label><div class="mt-3 grid gap-3 md:grid-cols-2"><div><label class="block mb-1">Внешнее название</label><input name="external_name" value="<?= htmlspecialchars($product['external_name'] ?? '') ?>" class="w-full border px-2 py-1 rounded"></div><div><label class="block mb-1">Внешний SKU</label><input name="external_sku" value="<?= htmlspecialchars($product['external_sku'] ?? '') ?>" class="w-full border px-2 py-1 rounded"></div><div class="md:col-span-2"><label class="block mb-1">Внешнее изображение (URL)</label><input name="external_image_path" type="url" value="<?= htmlspecialchars($product['external_image_path'] ?? '') ?>" placeholder="https://..." class="w-full border px-2 py-1 rounded"><p class="mt-1 text-xs text-gray-500">Если не задано, YML использует основное изображение товара.</p></div><div class="md:col-span-2"><label class="block mb-1">Внешнее описание</label><textarea name="external_description" rows="3" class="w-full border px-2 py-1 rounded"><?= htmlspecialchars($product['external_description'] ?? '') ?></textarea></div></div></fieldset>
  </div>
