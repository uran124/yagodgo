...
final class Florix24InboundService
{
    ...
    private function partner(array $p): array { if(!isset($p['berrygo_user_id']))return [null,[]];$q=$this->pdo->prepare("SELECT id,role,is_blocked,name FROM users WHERE id=?");$q->execute([(int)$p['berrygo_user_id']]);$u=$q->fetch(PDO::FETCH_ASSOC);if(!$u||(int)$u['is_blocked']||!in_array($u['role'],['partner','manager','admin'],true))return [null,[['code'=>'partner_not_available','message'=>'Партнер не назначен']]];return [['id'=>(int)$u['id'],'external_id'=>(string)($p['florix_user_id']??''),'name'=>(string)($p['name']??$u['name'])],[]]; }
}
