...
## 16. Статус внедрения на 11.05.2026 (по коду и миграциям)

Ниже — короткий срез по репозиторию, чтобы фиксировать, что уже внедрено относительно этого ТЗ.

### Уже реализовано

1. **Базовые supply-миграции применены и находятся в репозитории**:
   - `purchase_batches`, `stock_movements`, `purchase_batch_photos`;
   - supply-поля в `products`, `orders`;
   - дефолты pricing в `settings`;
   - роль закупщика/покупателя в `users`.
2. **Сервисный слой для supply-логики выделен**:
   - `PurchaseBatchService`, `StockService`, `PricingService`.
3. **Админ-контур для партий и закупок работает**:
   - отдельный контроллер партий (`PurchaseBatchesController`),
   - страницы списка/создания/карточки закупок.
4. **Заказы и каталог уже учитывают новую модель**:
   - есть обновления в `OrdersController`, `Order` и `ClientCatalogService`.
5. **Есть тестовое покрытие ключевых участков**:
   - unit/functional тесты для pricing, партий, остатков, заказов и админ-страниц.

### Что ещё не доведено до “операционного контура”

- Сквозная аналитика по марже партии и оборачиваемости остатков (от закупки до списания).
- Формализованный SLA/алерты на расхождения в остатках.
- Более строгие бизнес-ограничения на уровне БД (check/уникальности/инварианты по режимам продаж).

---

## 17. Следующие 5 шагов (приоритет)

1. **Закрыть инварианты остатков на уровне БД и сервиса**
   - Добавить жёсткие проверки, что `boxes_remaining = boxes_total - boxes_sold - boxes_written_off` и значения не уходят в минус.
   - Добавить транзакционные проверки в `PurchaseBatchService`/`StockService` для конкурентных изменений.

2. **Доделать финансовый отчёт по партии (P&L)**
   - На карточке партии показать: себестоимость, выручку, валовую маржу, списания, остаток в деньгах.
   - Добавить экспорт (CSV) для закупщика/руководителя.

3. **Усилить правила режима `discount_stock` сквозняком**
   - Проверить, что для “выгодного остатка” везде отключены бонусы/промокоды/доп.скидки (каталог, корзина, checkout, post-order).
   - Добавить интеграционные тесты на эти ограничения.

4. **Автоматизировать алерты по рискам остатков**
   - Уведомления в Telegram/админку о партиях с зависшим остатком, аномальной уценкой и расхождением движения/остатка.
   - Добавить ежедневный джоб с digest по “проблемным” партиям.

5. **Подготовить supply-ready релизный контур**
   - Финализировать runbook запуска supply-флоу (миграции, откат, smoke-check).
   - Добавить обязательный pre-release чек: миграции + supply smoke tests + ручной чек ключевых сценариев закупки.
