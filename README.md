...
- `config/telegram.example.php`
- `config/sms.example.php`
...
1. Сгенерировать новые ключи/токены у провайдеров (DB, Telegram, SMS, email).
2. Обновить секреты в секрет-хранилище окружения (не в git).
3. Перезапустить приложение и убедиться, что старт проходит без ошибок валидации конфигов.
4. Проверить отправку тестовых уведомлений: SMS, Telegram, email.
5. Отозвать старые ключи у провайдеров.
6. Проверить, что логи не содержат секретов в открытом виде.
...
## Перенос Telegram-бота на отдельный хостинг (VDS CZ)

Для обхода проблем доступности Telegram на RU-хостинге бот вынесен в отдельный каталог:

- `kraswebsite.ru/bots/berrygo/`

Важно:

- эта папка относится к **другому хостингу** (VDS в Чехии);
- основной сайт продолжает работать на текущем хостинге;
- деплой этой папки выполняется отдельно через файловый менеджер/на VDS.

Подробная инструкция по запуску и подключению webhook находится в:

- `kraswebsite.ru/bots/berrygo/README.md`.
