...
return [
    ...
    static function (string $method, string $uri, array $c): bool {
        if (!routeRegex('GET', '#^/buyer/purchases/(\d+)/pnl\.csv$#', $method, $uri, $m)) return false;
        requireBuyer(); (new App\Controllers\PurchaseBatchesController($c['pdo']))->exportPnlCsv((int)$m[1]); return true;
    },
];
