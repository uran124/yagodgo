...
return [
    ...
    static function (string $method, string $uri, array $c): bool {
        if (!routeExact('POST', '/admin/settings/integrations/florix24/inbound-token', $method, $uri)) return false;
        requireAdmin(); (new App\Controllers\Florix24TokenAdminController($c['pdo']))->create(); return true;
    },
    static function (string $method, string $uri, array $c): bool {
        if (!routeExact('POST', '/admin/settings/integrations/florix24/inbound-token/revoke', $method, $uri)) return false;
